import { soulCompute } from './soul-compute';

const plugin = {
  id: 'neshama-soul',
  name: 'Neshama Soul',
  description: '给 AI Agent 注入灵魂，基于 OCEAN 人格模型和 Valence-Arousal 情绪系统',
  register(api: { registerTool: (config: {
    name: string;
    description: string;
      ...
      async execute(_id: string, params: Record<string, unknown>) {
        try {
          const result = await soulCompute({
            ...
            ocean: params.ocean as { openness?: number; conscientiousness?: number; extraversion?: number; agreeableness?: number; neuroticism?: number },
