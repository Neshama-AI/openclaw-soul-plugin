...
export async function soulCompute(params: SoulComputeParams): Promise<SoulComputeResponse> {
  const apiKey = params.api_key || DEFAULT_API_KEY;
  const ocean = { ...DEFAULT_OCEAN, ...params.ocean };
  ...
  try {
    const response = await axios.post<SoulComputeResponse>(API_ENDPOINT, requestBody, {
      headers: {
...
export function generatePersonalityPrompt(response: SoulComputeResponse): string {
  return `【人格提示词】
